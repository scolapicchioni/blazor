...
namespace PhotoSharingExamples.Frontend.Core.Interfaces
{
    ...
    {
        Task<Photo> CreateAsync(string title, byte[] photoFile, string imageMimeType, string description, string userName, string tokenValue);
        Task<Photo> UpdateAsync(int id, string title, byte[] photoFile, string imageMimeType, string description, string tokenValue);
